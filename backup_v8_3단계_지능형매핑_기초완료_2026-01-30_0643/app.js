...
const MappingManager = {
	...
	findBestMatch(source, dbList) {
		...
		for (const db of dbList) {
			...
			// 1단계: 완전 일치 (상품명 정규화 대조 + 옵션 대조)
			if ((dbFullName === sName || dbPureName === sName) && dbOption.includes(sColor)) {
				return { product: db, status: 'success', similarity: 100 };
			}

			// 2단계: 상품명은 맞는데 색상이 애매한 경우 (퍼지 매칭)
			if (dbFullName === sName || dbPureName === sName || sName.includes(dbPureName)) {
				const colorSim = this.calculateSimilarity(sColor, dbOption);
				if (colorSim > 60) {
					const status = colorSim > 85 ? 'success' : 'warning';
					if (colorSim > bestMatch.similarity) {
						bestMatch = { product: db, status: status, similarity: colorSim };
					}
				}
			}
		}
