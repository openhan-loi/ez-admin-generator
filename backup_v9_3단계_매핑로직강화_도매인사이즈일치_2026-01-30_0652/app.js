	...
	renderWholesalerTags() {
		...
		this.wholesalers.forEach((wholesaler) => {
			...
			tag.addEventListener('click', (e) => {
				...
				this.renderWholesalerTags(); // 시각적 상태 갱신
			});
...
AppState.updateDBStats = function () {
	if (typeof DatabaseManager !== 'undefined' && DatabaseManager.getAll) {
		DatabaseManager.getAll((products) => {
...
const MappingManager = {
	...
	async startAutoMapping() {
		...
		DatabaseManager.getAll(async (dbProducts) => {
			if (dbProducts.length === 0) {
				UIController.showToast(
					'등록된 제품 DB가 없습니다. 2단계에서 DB를 먼저 구축해주세요.',
					'error',
				);
				return;
			}
			...
			sourceData.forEach((item, idx) => {
				...
				this.mappings.push({
					...
					status: match.status, // 'success', 'warning', 'danger'
			...
			this.updateSummary();
			UIController.showToast('자동 매핑이 완료되었습니다. 결과를 검토해주세요!', 'success');
		});
	},
	...
	renderMappingResults() {
		...
		this.mappings.forEach((m, idx) => {
			const tr = document.createElement('tr');
			const statusBadge = `<span class="badge badge-${m.status}">${m.status === 'success' ? '매칭 성공' : m.status === 'warning' ? '검토 필요' : '매칭 실패'}</span>`;
			...
			tr.innerHTML = `
				...
				<td>
					<div class="mapping-row-info">
						<strong>${m.source.productName}</strong>
						<span class="sub-text">컬러: ${m.source.color}</span>
					</div>
				</td>
				<td>${dbInfo}</td>
				<td>${sourceQty}</td>
				<td>
					<button class="btn btn-secondary btn-sm" onclick="MappingManager.openManualSearch(${idx})">수동 매칭</button>
				</td>
			`;
			tbody.appendChild(tr);
		});
	...
	openManualSearch(idx) {
		...
		const searchName = prompt(
			'검색할 상품명이나 코드를 입력하세요:',
			mapping.source.productName.split('-').pop(),
		);
		...
		DatabaseManager.getAll((dbProducts) => {
			const results = dbProducts.filter(
				(p) =>
					...
					p.productCode.includes(searchName) ||
					p.barcode.includes(searchName),
			);

			if (results.length === 0) {
				UIController.showToast('검색 결과가 없습니다.', 'warning');
				return;
			}

			// 간단한 다이얼로그 대신 prompt로 선택 (추후 UI 개선 가능)
			let msg = '매칭할 상품의 번호를 입력하세요:\n';
			results.slice(0, 10).forEach((p, i) => {
				msg += `${i + 1}. [${p.productCode}] ${p.productName} (${p.optionName})\n`;
			});

			const choice = prompt(msg);
			if (choice && !isNaN(choice) && results[choice - 1]) {
				const selected = results[choice - 1];
				this.mappings[idx].target = selected;
				this.mappings[idx].status = 'success';
				this.renderMappingResults();
				this.updateSummary();
				UIController.showToast('수동 매칭이 완료되었습니다.', 'success');
			}
		});
	},

	downloadMappingDebug() {
		if (this.mappings.length === 0) {
			UIController.showToast('매핑된 데이터가 없습니다.', 'warning');
			return;
		}
		...
		this.mappings.forEach((m) => {
			const statusText = m.status === 'success' ? '성공' : m.status === 'warning' ? '주의' : '실패';
			const row = [
				statusText,
				m.source.wholesaler,
				`"${m.source.productName}"`,
				`"${m.source.color}"`,
				`"${Object.keys(m.source.quantities)[0] || ''}"`,
				m.target ? m.target.productCode : '',
				m.target ? `"${m.target.productName}"` : '',
				m.target ? `"${m.target.optionName}"` : '',
				m.target ? m.target.barcode : '',
		...
		const a = document.createElement('a');
		a.href = url;
		a.download = `매핑디버그_${new Date().getTime()}.csv`;
		a.click();
		UIController.showToast('디버그 파일이 다운로드되었습니다.', 'success');
	},
};
