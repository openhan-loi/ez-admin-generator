	...
	async handleFiles(files) {
		...
		for (let i = 0; i < files.length; i++) {
			...
			if (file.size > 10 * 1024 * 1024) {
				UIController.showToast(`${file.name}: 10MB를 초과합니다.`, 'error');
				continue;
			}
...
const ExcelAnalyzer = {
	...
	detectTableType(data) {
		...
		if (firstRow[11] && String(firstRow[11]).includes('품명')) return 'right';
		if (firstRow[2] && String(firstRow[2]).includes('품명')) return 'left';
		return 'unknown';
	},
	...
	extractFromLeftTable(data, sheetName, fileName, wholesaler, results) {
		...
		for (let i = 2; i < data.length; i++) {
			...
			const rawSize = row[SIZE_COL] ? String(row[SIZE_COL]).trim() : null;
			const size = rawSize ? rawSize.replace(/"/g, '') : null;
			const qty = row[QTY_COL] ? parseInt(row[QTY_COL]) : 0;
	...
	extractFromRightTable(data, sheetName, fileName, wholesaler, results) {
		...
		const COLOR_COL = 12;
		const SIZE_ROW_IDX = 1;
		const DATA_START_IDX = 2;

		const sizeRow = data[SIZE_ROW_IDX];
		if (!sizeRow) return;
