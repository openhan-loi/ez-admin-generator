...
const DatabaseManager = {
	...
	// 매핑 기억 관련 함수들
	saveMappingMemory(mappingKey, productCode) {
		if (!this.db) return;
		const transaction = this.db.transaction(['mappingMemory'], 'readwrite');
		const store = transaction.objectStore('mappingMemory');
		store.put({ mappingKey, productCode, timestamp: new Date().getTime() });
	},

	getMappingMemory(callback) {
...
const MappingManager = {
				...
				sourceData.forEach((item, idx) => {
					...
					if (remembered) {
						...
						if (dbProduct) {
							...
							this.addFeedItem(item, dbProduct, true); // 피드 추가 (기억 활용)
							return;
						}
					}
					...
					// C. 만약 자동 매칭 성공이면 기억에 저장 및 피드 노출
					if (match.status === 'success' && match.product) {
						DatabaseManager.saveMappingMemory(mappingKey, match.product.productCode);
	...
	addKeyword(keyword) {
		...
		if (currentVal.includes(keyword)) return;

		input.value = currentVal ? `${currentVal} ${keyword}` : keyword;
	...
	openManualSearch(idx) {
		...
		// 2. 초기 검색어 설정 및 모달 노출
		input.value = mapping.source.productName.split('-').pop();
	...
	renderKeywordHelpers(mapping) {
		...
		helperArea.innerHTML = `
			...
			${this.lastQuery ? `<button class="btn-text-sm" onclick="MappingManager.applyLastQuery()">🔄 최근 검색어: "${this.lastQuery}"</button>` : ''}
		`;
	},
	...
	executeManualSearch() {
		...
		this.lastQuery = query; // 최근 검색어 기억
	...
	selectProduct(productCode) {
		DatabaseManager.getAll((dbProducts) => {
			...
			if (selected) {
				...
				// 1. 수동 매칭 결과 기억 저장
				const mappingKey = `${item.wholesaler}|${item.productName}|${item.color}|${Object.keys(item.quantities)[0] || ''}`;
				DatabaseManager.saveMappingMemory(mappingKey, selected.productCode);
